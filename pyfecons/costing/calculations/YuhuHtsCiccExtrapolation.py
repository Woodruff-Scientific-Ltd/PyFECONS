...
yuhudata = {
    ...
    "z_av(m)": [0] * 54,
    "dr(m)": [0.49] * 6 + [0.69] * 6 + [0.84] * 6 + [0.98] * 6 + [1.09] * 6 + [1.19] * 6 + [1.29] * 6 + [
        1.38] * 6 + [1.46] * 6,
    "dz(m)": [0.49] * 6 + [0.69] * 6 + [0.84] * 6 + [0.98] * 6 + [1.09] * 6 + [1.19] * 6 + [1.29] * 6 + [
        1.38] * 6 + [1.46] * 6,
...
class YuhuHtsCiccExtrapolation:
    def __init__(self, magnet: Magnet, override_checks: bool):
        ...
        cableCurrentCICC_interp = LinearNDInterpolator(interp_data[['B_mag', 'r_av(m)']].values,
                                                       interp_data['Single cable I (A)'])
        ...
        self.x_new = np.array([[magnet.auto_cicc_b, magnet.auto_cicc_r]])  # Adjusted for your new data
        self.override_checks = override_checks  # Set this to True to bypass the range checks
        self.r_turns = interpolate_or_extrapolate(rTurnsCICC_interp, models['R_Turns'], self.x_new,
                                                  override_checks=override_checks)
        self.z_turns = interpolate_or_extrapolate(zTurnsCICC_interp, models['Z_Turns'], self.x_new,
                                                  override_checks=override_checks)
        self.cable_current = interpolate_or_extrapolate(cableCurrentCICC_interp, models['Single cable I (A)'],
                                                        self.x_new, override_checks=override_checks)
        self.dr = interpolate_or_extrapolate(drCICC_interp, models['dr(m)'], self.x_new,
                                             override_checks=override_checks)
        self.dz = interpolate_or_extrapolate(dzCICC_interp, models['dz(m)'], self.x_new,
                                             override_checks=override_checks)
