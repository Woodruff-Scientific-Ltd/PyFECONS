...
def compute_engineering_costs(primary_structure: PrimaryStructure, power_table: PowerTable) -> M_USD:
    IN = primary_structure
    p_et = power_table.p_et

    # standard engineering costs
    engineering_costs = M_USD(scaled_cost(IN.analyze_costs, p_et, IN.learning_credit)
...
def compute_fabrication_costs(primary_structure: PrimaryStructure, power_table: PowerTable) -> M_USD:
    IN = primary_structure
    p_et = power_table.p_et

    # standard fabrication costs
    fabrication_costs = M_USD(scaled_cost(IN.unit1_fab_costs, p_et, IN.learning_credit)
