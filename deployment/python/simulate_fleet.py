...
from utils import (
    energyTWhToCarbonOutputKG,
    capacityMWToEnergyTWh,
    s_curve,
    log_memory_usage,
    get_usa_maps,
    kgToGTons,
...
def simulatePlants(
    ...
    if percent_fusion > 1:
        percent_fusion = percent_fusion * 0.01
...
def extend_datasets(totalEnergy, totalCapacity, addCapDiffProp, end_year):
    ...
    predicted_energy += 2 * (
        totalEnergy[totalEnergy["year"] == 2040]["EnergyBillkWh"].values[0]
        - predicted_energy[0]
    )
...
def set_up_fusion_and_add_to_addCapDiffProp(
    addCapDiffProp,
    addWeightedCF,
    after_year,
    start_year,
    ...
    percent_fusion = s_curve(
        T_ADOPT=T_ADOPT,
        k=percent_fusion,
        t=(np.arange(1, len(addCapDiffProp) + 1) - after_year_ind),
    )

    percent_fusion[np.isnan(percent_fusion)] = percent_fusion[0]
    percent_fusion[percent_fusion < 0] = percent_fusion[0]
    percent_fusion = np.concatenate(
        [np.zeros(after_year_ind - 1), percent_fusion[after_year_ind - 1 :]]
    )
    ...
    if "All" in to_replace or "all" in to_replace:
        ...
        if not percent_fusion.index.equals(temp_sum.index):
            raise ValueError(
                "Indices of percent_fusion and temp_sum do not match after alignment"
            )
...
def miscellaneous_preparation(summary, addWeightedCF, typeChars, totalCapacity, fleet0):
    ...
    summary["demandGrowth"] = np.maximum(
        0, np.diff(totalCapacity["capacity"], prepend=0)
    )
...
def simulate_year(
    ...
    # Calculate energy
    fleet_t["energy"] = capacityMWToEnergyTWh(
        fleet_t["capacity"], fleet_t["type"], typeChars
    )

    # Calculate carbon output
    fleet_t["carbonOutput"] = energyTWhToCarbonOutputKG(
        fleet_t["energy"],
        fleet_t["type"],
        typeChars,
        percent_CCS if year > afterYear else 0,
    )
    ...
    summary.loc[year, "NGCCDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "NGCC"]["capacity"]
    )
    summary.loc[year, "WindDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "Wind"]["capacity"]
    )
    summary.loc[year, "NuclearDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "Nuclear"]["capacity"]
    )
    summary.loc[year, "PetroleumDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "Petroleum"]["capacity"]
    )
    summary.loc[year, "NGSTDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "NGST"]["capacity"]
    )
    summary.loc[year, "NGCTDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "NGCT"]["capacity"]
    )
    summary.loc[year, "PVDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "PV"]["capacity"]
    )
    summary.loc[year, "HydroDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "Hydroelectric"]["capacity"]
    )
    summary.loc[year, "CoalDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "Coal"]["capacity"]
    )
    summary.loc[year, "fusionDearth"] = np.sum(
        retiring_plants[retiring_plants["type"] == "NuclearFusion"]["capacity"]
    )
    summary.loc[year, "replacementRetirements"] = np.sum(retiring_plants["capacity"])

    # Remove retiring plants from the fleet
    remaining_indices = np.setdiff1d(
        np.arange(fleet_t.shape[0]), np.where(retiring_indices)[0]
    )
    ...
    dearth_t_cap = (
        dearth_t_energy / (yearlyWeightedCF.loc[year] * 10**-2 * 8760)
    ) * 10**6
    ...
    summary.loc[year, "NGCCGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "NGCC"]["capacity"]
    )
    summary.loc[year, "WindGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "Wind"]["capacity"]
    )
    summary.loc[year, "NuclearGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "Nuclear"]["capacity"]
    )
    summary.loc[year, "PetroleumGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "Petroleum"]["capacity"]
    )
    summary.loc[year, "NGSTGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "NGST"]["capacity"]
    )
    summary.loc[year, "PVGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "PV"]["capacity"]
    )
    summary.loc[year, "HydroGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "Hydroelectric"]["capacity"]
    )
    summary.loc[year, "CoalGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "Coal"]["capacity"]
    )
    summary.loc[year, "NGCTGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "NGCT"]["capacity"]
    )
    summary.loc[year, "fusionGrowth"] = np.sum(
        fleet_t1[fleet_t1["type"] == "NuclearFusion"]["capacity"]
    )
    summary.loc[year, "carbonIntensity"] = np.sum(fleet_t["carbonOutput"]) / (
        np.sum(fleet_t["energy"]) * 10**9
    )
    summary.loc[year, "totalCarbon"] = np.sum(kgToGTons(fleet_t["carbonOutput"]))
    summary.loc[year, "newCap"] = (
        np.sum(fleet_t1[fleet_t1["isNew"]]["capacity"]) / 10**3
    )
    ...
    if num_plants_types.sum() > 0:
        ...
        new_plants["capacity"] = np.maximum(
            np.random.normal(
                typeChars.loc[new_plants["type"], "AvgCapacityMW"].values,
                typeChars.loc[new_plants["type"], "StdDevCapacityMW"].values,
            ),
            2,
        )
        new_plants["locCode"] = np.random.choice(
            retiring_plants["locCode"], len(new_plants), replace=True
        )
        new_plants["age"] = 1
        new_plants["year"] = year
        new_plants["seqNum"] = np.arange(
            fleet_t["seqNum"].max() + 1, fleet_t["seqNum"].max() + 1 + len(new_plants)
        )
        new_plants["expRet"] = rsnorm(
            len(new_plants),
        ...
        new_plants["energy"] = capacityMWToEnergyTWh(
            new_plants["capacity"], new_plants["type"], typeChars
        )
        new_plants["carbonOutput"] = energyTWhToCarbonOutputKG(
            new_plants["energy"],
            new_plants["type"],
            typeChars,
            percent_CCS if year > afterYear else 0,
        ...
        for _ in range(5):
            to_change = np.isnan(new_plants["expRet"]) | (
                new_plants["expRet"] < new_plants["age"]
            )
            new_plants["expRet"][to_change] = rsnorm(
                to_change.sum(),
        ...
        new_plants["expRet"][nuclear_types] = (
            np.random.binomial(1, 0.5, nuclear_types.sum()) * 20 + 40
        )
        new_plants["long"] = usaMap2.loc[
            new_plants["locCode"], "long"
        ].values + np.random.normal(0, 0.05, len(new_plants))
        new_plants["lat"] = usaMap2.loc[
            new_plants["locCode"], "lat"
        ].values + np.random.normal(0, 0.05, len(new_plants))
    ...
    summary.loc[year, "newFusionAdditions"] = np.sum(
        new_plants[new_plants["type"] == "NuclearFusion"]["capacity"]
    )
...
def create_graphs(fleet_t, graph_output_dir, usaMap2, year):
    ...
    g = (
        ggplot(plot_data[plot_data["carbonOutput"] < 1e09])
        + geom_point(
            aes(x="capacity", y="carbonOutput", color="type"), size=1.5, alpha=0.2
        )
        + theme_void()
        + labs(y="Carbon Output", x="Capacity")
        + coord_cartesian(xlim=(0, 500), ylim=(0, 1e09))
    )
    ...
    p_combined = (
        p
        + geom_point(
            aes(x="long", y="lat", color="type", size="capacity", shape="isFusion"),
            data=plot_data,
            alpha=0.4,
        )
