...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Plot capacity change graph for simulation output"
    )
    parser.add_argument(
        "--input_csv",
        type=str,
        default="out/run/simulation_summary.csv",
        help="Simulation results csv file.",
    )
    parser.add_argument(
        "--out_dir", type=str, default="out/run", help="Output directory to save file"
    )
    parser.add_argument(
        "--filename",
        type=str,
        default="capacity_graph.png",
        help="Output directory to save file",
    )
    parser.add_argument(
        "--start_year", type=int, default=None, help="Start year of graph"
    )
