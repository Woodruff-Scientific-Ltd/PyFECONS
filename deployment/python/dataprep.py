...
typeChars = pd.read_excel(
    "data/generators_new.xlsx", sheet_name="CF_Operating_Retired_Data"
)
...
typeChars.loc[["WoodOtherBiomass", "OtherGas"], "initialCarbonPerMW"] = (
    210  # assumption
)
...
typeChars["initialCarbonPerMW"] = (
    typeChars["initialCarbonPerMW"] * 8760 * 10 ** (-3) * 10**3
)  # first year and in kg's and in MW
...
retiredGen = pd.read_excel(
    "data/generators_new.xlsx", sheet_name="Retired and Canceled 2"
)
retiredGen.columns = retiredGen.columns.str.replace(" ", "").str.replace(
    "[.]", "", regex=True
)
...
retiredGen["RetirementAge"] = pd.to_numeric(
    retiredGen["RetirementAge"], errors="coerce"
)
...
typeChars["skewness"] = retiredGen.groupby("Technology")["RetirementAge"].apply(
    fit_skewnorm
)

# set the skewness of fusion
typeChars.loc["NuclearFusion", "skewness"] = 0.3  # adapted from fission
...
addCap = pd.read_excel(
    "data/AEOprojections.xlsx", sheet_name="Sheet3", header=None, index_col=0
)
...
ongsProps = (
    typeChars.loc[ongs, "TotalCapacityMW"]
    / typeChars.loc[ongs, "TotalCapacityMW"].sum()
)
ctdProps = (
    typeChars.loc[ctd, "TotalCapacityMW"] / typeChars.loc[ctd, "TotalCapacityMW"].sum()
)
renProps = np.array(
    [0.006, 0.06, 0.015, 0.188, 0.009, 0.798, 0.06]
)  # comes from: typeChars[renewables, "TotalCapacityMW"]/sum(typeChars[renewables, "TotalCapacityMW"])
...
addCap.rename(
    columns={"CombinedCycle": "NGCC", "NuclearPower": "Nuclear"}, inplace=True
)
addCap.drop(
    columns=[
        "OilandNaturalGasSteam",
        "CombustionTurbineDiesel",
        "RenewableSources",
        "PumpedStorage",
        "FuelCells",
...
addCapDiff["TotalTest"] = addCapDiff.sum(
    axis=1
)  # this is to check that it all sums to 1
...
currentGen = currentGen[
    ~currentGen["type"].isin(
        ["Batteries", "Flywheels", "HydroelectricPumpedStorage", "AllOther"]
    )
]
...
currentGen.loc[currentGen["capacity"].isna(), "capacity"] = currentGen.loc[
    currentGen["capacity"].isna(), "type"
].map(typeChars["AvgCapacityMW"])
currentGen["energy"] = (
    currentGen["capacity"]
    * currentGen["type"].map(typeChars["CapFactor"])
    * 8760
    / 10**8
)
currentGen["carbonOutput"] = (
    currentGen["energy"]
...
currentGen["locCode"] = (
    currentGen["state"].str.lower() + "," + currentGen["county"].str.lower()
)
...
mismatched_locs = currentGen.loc[
    ~currentGen["locCode"].isin(usaMap["locCode"]), "locCode"
]
...
currentGen["long"] = currentGen["geometry"].apply(
    lambda geom: geom.centroid.x
) + np.random.normal(0, 0.05, len(currentGen))
currentGen["lat"] = currentGen["geometry"].apply(
...
typeChars["StdDevRetAge"] = typeChars["StdDevRetAge"].apply(
...
typeChars.loc[
    ["MunicipalSolidWaste", "Geothermal", "SolarThermal"],
    ["AvgRetAge", "StdDevRetAge", "skewness"],
...
typeChars.loc["WoodOtherBiomass", ["AvgRetAge", "StdDevRetAge", "skewness"]] = (
    typeChars.loc["Petroleum", ["AvgRetAge", "StdDevRetAge", "skewness"]]
)
...
try:
    currentGen["expRet"] = skewnorm.rvs(
        size=len(currentGen),
        a=currentGen["skewness"],
...
currentGen.loc[currentGen["type"].eq("Nuclear"), "expRet"] = (
    np.random.binomial(1, 0.85, size=currentGen["type"].eq("Nuclear").sum()) * 20 + 40
)

if tempC["type"].eq("OtherGas").sum() > 0:
    other_gas_indices = tempC[tempC["type"] == "OtherGas"].index
    currentGen.loc[other_gas_indices, "expRet"] = tempC.loc[
        other_gas_indices, "age"
    ] + np.random.uniform(0, 10, size=other_gas_indices.size)
...
totalCapacity = pd.DataFrame(
    {"year": range(2014, 2042), "capacity": totalCapacity.columns.astype(float)}
)
